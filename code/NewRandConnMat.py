            ...
            if same_minicolumn(i,j):
                ...
                # FROM LAYER2/3 -> LAYER5
                    ...
                    if both_exc(i,j):
                        ...
                        conn_matrix[j][i]= flip(0.20,i)
                        countB = check_count(countB, conn_matrix[j][i])
            elif same_hypercolumn(i,j) and not same_minicolumn(i,j):
                conn_matrix[j][i] = flip(0.3,i)
                countQ = check_count(countQ,conn_matrix[j][i])
            #"""
...
print (count23+count4+count5+countQ)/float(g)*100.
print float(float(countAz)/float(g))*100.
