...
x1 = random.choice(tuple([i for i in pm.layers5 if i in pm.split_hc[2] and i in pm.exc_nrns_set]))
x2 = random.choice(tuple([i for i in pm.layers5 if i in pm.split_hc[2] and i in pm.inh_nrns_set]))

nest.SetStatus([Nestrons[x1]],{'V_th':1000.,'V_reset':500.})
nest.Connect(vm,[Nestrons[x1]])
nest.SetStatus([Nestrons[x2]],{'V_th':1000.,'V_reset':500.})
nest.Connect(vm,[Nestrons[x2]])
...
vv = np.loadtxt('./data/'+prefix+'voltmeter-2884-0.dat')
